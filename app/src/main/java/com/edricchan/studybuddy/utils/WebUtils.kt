...
class WebUtils(val context: Context) {
	...
	fun launchUri(uri: Uri) {
		...
		launchUri(uri, preferences.getBoolean(Constants.prefUseCustomTabs, true),
	 ...
	 */
	fun launchUri(uri: Uri, useCustomTabs: Boolean = true,
	 ...
	 * @param useAppColorScheme Whether to respect the app's color/colour scheme
	 */
	fun launchUri(uri: String, useCustomTabs: Boolean = true,
	              useAppColorScheme: Boolean = true) {
